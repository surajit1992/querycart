...
//echo $protocol;
$post_url = admin_url( 'admin.php?page=order-plugin%2Fproduct-enquiry-fields.php', $protocol );
if(!empty($_POST))
{
    $post = $_POST;
    if(!empty($post["action"]))
    {
        ...
        if($action == "add")
        {
            ...
            $query = "insert `".$wpdb->prefix."order_meta` set $query_builder";
            $wpdb->query( $wpdb->prepare( $query ) );
			$insert_id = $wpdb->insert_id;
			update_field_order($wpdb->prefix."order_meta",$insert_id,$insert_id,"field_meta_id");
            echo '<div id="message" class="updated notice notice-success is-dismissible"><p>Query Meta Inserted.</p><button type="button" class="notice-dismiss"><span class="screen-reader-text">Dismiss this notice.</span></button></div>';
...
$fetch_query = "select * from `".$wpdb->prefix."order_meta`";
$results = $wpdb->get_results($fetch_query);
foreach($results as $single_result)
{
	$id_arr[] = $single_result->field_meta_id;
}
$fetch_query = "select * from `".$wpdb->prefix."order_meta`  order by `field_order` asc"; //where `userid` = '$user->ID'
$fetch_query_count = "select COUNT(*) from `".$wpdb->prefix."order_meta`";
...
<div class="wrap">
    <h1 class="wp-heading-inline">Product Fields</h1>
    ...
    if($wpdb->get_var($fetch_query_count)>0)
    {
		...
		if(!empty($order))
		{
			...
			{
				$id_arr = "$single_result->field_meta_id";
				$new_result[$id_arr] = $single_result;
			}
			unset($results);
			$order_arr = explode(",",$order);
			foreach($order_arr as $sor_arr)
			{
				$results[]=$new_result[$sor_arr];
			}
		}
*/
		?>
		...
		<?php
					...
					{
						echo "<p style=color:red;><a>Can Not Be Deleted</a></p>";
					}
					//echo "</li>";
